# MongoDB Sharding with Replication and Redis Caching using Docker Compose

Настройка кластера MongoDB в режиме шардирования с репликацией и Redis кэшированием в Docker Compose.
...
- **Redis Caching:** Кэширование запросов для ускорения ответов API
- **Cache TTL:** Настраиваемое время жизни кэша (по умолчанию 60 секунд)
- **Intelligent Caching:** Автоматическое кэширование часто запрашиваемых данных
...
### 4. Redis Cache (`redis`)
- **Port:** 6379
- **Role:** Кэширование запросов для повышения производительности API
- **Persistence:** AOF (Append Only File) для надежности данных
- **Volume:** `redis_data` для хранения данных кэша

### 5. Mongos Router (`mongos_router`)
- **Port:** 27020
...
### 6. Mongos Initialization (`mongos_init`)
...
- **Features:**
  ...
  - **Redis Caching:** Кэширование ответов API с TTL 60 секунд
  - **Cache Status:** Отображение статуса кэширования в API ответах
...
3. Запускает Redis для кэширования.
4. Запускает маршрутизатор mongos.
5. Запускает скрипт инициализации для:
   - Инициализации replica sets
   - Добавления шардов в кластер
   - Включения шардирования
6. Запускает API Python с поддержкой кэширования.
